...
Luma 是一个面向 macOS 的本地文件空间观察工具。它扫描用户明确选择的目录，在本机建立 SQLite 索引，并展示空间分类、最大文件、重复文件候选、开发项目与可解释的复查线索。默认扫描只读取路径和元数据；用户主动点击“查找重复”后，Luma 才会读取候选文件内容并计算本地 BLAKE3 哈希，全程不上传数据、不修改原文件。
...
src/                    前端
  ...
  components/           UI 组件（扫描控制、进度、概览、分类、最大文件、发现、重复、项目、历史）
...
当前版本暂不启用自动更新：发布流程不生成 updater 清单，也不在后台联网检查版本。待配置 Tauri updater 签名密钥和公钥后，再单独恢复自动更新链路。
